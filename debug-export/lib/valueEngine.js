...
export const calculateVaultPortfolio = ({
  ...
  const rawAssets = Object.entries(ownership)
    .map(([key, value]) => {
      ...
      const unitValue = Number.isFinite(Number(card.value)) ? Number(card.value) : 0;
  ...
  const vaultAssets = assets.map((asset) => {
    ...
    const unitValue = Number.isFinite(Number(selected.quote?.value)) ? Number(selected.quote.value) : 0;
