      ...
      <Modal visible={filtersOpen} transparent animationType="slide" onRequestClose={() => setFiltersOpen(false)}>
        <View style={styles.modalBackdrop}>
          <View style={styles.filterSheet}>
            ...
            <View style={styles.filterChips}>{['All', ...Array.from(new Set(CARD_LIBRARY.map((card) => card.rarity).filter(Boolean))).slice(0, 8)].map((value) => <TouchableOpacity key={value} style={[styles.filterChip, rarityFilter === value && styles.filterChipOn]} onPress={() => setRarityFilter(value)}><Text style={[styles.filterChipText, rarityFilter === value && styles.filterChipTextOn]}>{value}</Text></TouchableOpacity>)}</View>
