...
export const getLiveSetValuations = async (set) => {
  ...
  availableTiers.forEach((tier) => {
    ...
    requirements.forEach((requirement) => {
      const price = requirementPrice(requirement, book.byNumber.get(numberKey(requirement.number)) || []);
      if (price == null) return;
      value += price;
      priced += 1;
      requirementPrices[String(requirement.collectibleKey || requirement.id)] = price;
      livePriced += 1;
    });
    valuations[tier] = {
      value,
